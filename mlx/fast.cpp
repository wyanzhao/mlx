...
std::vector<array> fused_shortconv_step(
    const array& bcx,
    const array& state,
    const array& weight,
    StreamOrDevice s_ /* = {} */) {
  if (bcx.ndim() != 3 || bcx.shape(1) != 1 || bcx.shape(-1) % 3 != 0) {
    std::ostringstream msg;
    msg << "[fused_shortconv_step] bcx must have shape [B, 1, 3C], got "
        << bcx.shape() << ".";
    throw std::invalid_argument(msg.str());
  }

  int batch_size = bcx.shape(0);
  int channels = bcx.shape(-1) / 3;
  if (state.ndim() != 3 || state.shape(0) != batch_size ||
      state.shape(-1) != channels || state.shape(1) < 1) {
    std::ostringstream msg;
    msg << "[fused_shortconv_step] state must have shape [B, K-1, C] with "
           "K >= 2, got "
        << state.shape() << ".";
    throw std::invalid_argument(msg.str());
  }

  int kernel_size = state.shape(1) + 1;
  if (weight.ndim() != 3 || weight.shape(0) != channels ||
      weight.shape(1) != kernel_size || weight.shape(2) != 1) {
    std::ostringstream msg;
    msg << "[fused_shortconv_step] weight must have shape [C, K, 1], got "
        << weight.shape() << ".";
    throw std::invalid_argument(msg.str());
  }

  auto out_type = bcx.dtype();
  if (state.dtype() != out_type || weight.dtype() != out_type) {
    std::ostringstream msg;
    msg << "[fused_shortconv_step] bcx, state, and weight must have the same "
           "dtype, got "
        << bcx.dtype() << ", " << state.dtype() << ", and " << weight.dtype()
        << ".";
    throw std::invalid_argument(msg.str());
  }
  if (out_type != float32 && out_type != float16 && out_type != bfloat16) {
    std::ostringstream msg;
    msg << "[fused_shortconv_step] inputs must have dtype float32, float16, "
           "or bfloat16, got "
        << out_type << ".";
    throw std::invalid_argument(msg.str());
  }
  ...
  auto s = to_stream(s_);
  auto fallback = [channels, kernel_size, s](const std::vector<array>& inputs) {
    auto gates = split(inputs[0], 3, -1, s);
    auto bx = multiply(gates[0], gates[2], s);
    auto sequence = concatenate({inputs[1], bx}, 1, s);
    auto next_state = slice(
        sequence, {0, 1, 0}, {sequence.shape(0), kernel_size, channels}, s);
    auto conv = conv1d(sequence, inputs[2], 1, 0, 1, channels, s);
    auto y = multiply(gates[1], conv, s);
    return std::vector<array>{std::move(y), std::move(next_state)};
  };

  std::vector<array> inputs{bcx, state, weight};
  if (!FusedShortConvStep::use_fallback(s)) {
    auto y_shape = bcx.shape();
    y_shape.back() = channels;
    return array::make_arrays(
        {std::move(y_shape), state.shape()},
        {out_type, out_type},
        std::make_shared<FusedShortConvStep>(s, fallback),
        std::move(inputs));
  }
  return fallback(inputs);
}

// Must match MOE_ROUTE_MAX_EXPERTS / MOE_ROUTE_MAX_TOPK in
// backend/metal/kernels/moe_route.metal.
int moe_route_max_experts() {
  return 1024;
}
int moe_route_max_topk() {
  return 32;
}

bool MoERoute::is_equivalent(const Primitive& other) const {
  const MoERoute& a_other = static_cast<const MoERoute&>(other);
  return top_k_ == a_other.top_k_ &&
      routed_scaling_ == a_other.routed_scaling_ &&
      norm_topk_prob_ == a_other.norm_topk_prob_;
}

std::vector<array> moe_route(
    const array& x,
    ...
    StreamOrDevice s_ /* = {} */) {
  if (x.ndim() < 1) {
    throw std::invalid_argument("[moe_route] x must have at least one axis.");
  }
  if (gate_weight.ndim() != 2 || gate_weight.shape(-1) != x.shape(-1)) {
    std::ostringstream msg;
    msg << "[moe_route] gate_weight must have shape [num_experts, "
        << x.shape(-1) << "], got " << gate_weight.shape() << ".";
    throw std::invalid_argument(msg.str());
  }
  int num_experts = gate_weight.shape(0);
  if (top_k < 1 || top_k > num_experts) {
    std::ostringstream msg;
    msg << "[moe_route] top_k must be in [1, " << num_experts << "], got "
        << top_k << ".";
    throw std::invalid_argument(msg.str());
  }
  // The kernel holds probs in a fixed threadgroup array and the selection in
  // fixed thread-private arrays, so these caps are memory safety, not style:
  // without them a larger router writes past both.
  if (num_experts > moe_route_max_experts()) {
    std::ostringstream msg;
    msg << "[moe_route] num_experts must be at most "
        << moe_route_max_experts() << ", got " << num_experts
        << "; the fused kernel stores the router probabilities in threadgroup"
           " memory of that size.";
    throw std::invalid_argument(msg.str());
  }
  if (top_k > moe_route_max_topk()) {
    std::ostringstream msg;
    msg << "[moe_route] top_k must be at most " << moe_route_max_topk()
        << ", got " << top_k
        << "; the fused kernel holds the selection in thread-private arrays of"
           " that size.";
    throw std::invalid_argument(msg.str());
  }
  if (expert_bias &&
      (expert_bias->ndim() != 1 || expert_bias->shape(0) != num_experts)) {
    std::ostringstream msg;
    msg << "[moe_route] expert_bias must have shape [" << num_experts
        << "], got " << expert_bias->shape() << ".";
    throw std::invalid_argument(msg.str());
  }
  // Both dtype rules are checked rather than coerced: an astype here would be
  // an extra dispatch per call, which is the whole cost this op exists to
  // remove. They are checked at all because they are silent otherwise -- a
  // bfloat16 expert_bias read as float32 produces plausible-looking garbage
  // that survives every isolated test and only shows up as wrong experts on a
  // real checkpoint.
  if (gate_weight.dtype() != float32) {
    std::ostringstream msg;
    msg << "[moe_route] gate_weight must be float32, got "
        << gate_weight.dtype() << ".";
    throw std::invalid_argument(msg.str());
  }
  if (expert_bias && expert_bias->dtype() != x.dtype()) {
    std::ostringstream msg;
    msg << "[moe_route] expert_bias must have the same dtype as x (" << x.dtype()
        << "), got " << expert_bias->dtype() << ".";
    throw std::invalid_argument(msg.str());
  }
  auto out_type = x.dtype();
  if (out_type != float32 && out_type != float16 && out_type != bfloat16) {
    std::ostringstream msg;
    msg << "[moe_route] x must have dtype float32, float16, or bfloat16, got "
        << out_type << ".";
    throw std::invalid_argument(msg.str());
  }

  auto s = to_stream(s_);
  auto fallback = [top_k, routed_scaling, norm_topk_prob, s](
                      const std::vector<array>& inputs) {
    // The reference composition, op for op: selection sees the bias, the
    // emitted weights do not.
    auto probs = sigmoid(
        matmul(astype(inputs[0], float32, s), transpose(inputs[1], s), s), s);
    auto selection = inputs.size() == 3
        ? add(probs, astype(inputs[2], float32, s), s)
        : probs;
    auto order = argpartition(selection, -top_k, -1, s);
    auto start = Shape(order.ndim(), 0);
    start.back() = order.shape(-1) - top_k;
    auto inds = slice(order, start, order.shape(), s);
    auto scores = take_along_axis(probs, inds, -1, s);
    if (norm_topk_prob) {
      scores = divide(
          scores,
          add(sum(scores, -1, true, s), array(1e-6f, float32), s),
          s);
    }
    scores = multiply(scores, array(routed_scaling, float32), s);
    return std::vector<array>{
        astype(inds, uint32, s), astype(scores, inputs[0].dtype(), s)};
  };

  std::vector<array> inputs{x, gate_weight};
  if (expert_bias) {
    inputs.push_back(*expert_bias);
  }
  if (!MoERoute::use_fallback(s)) {
    auto shape = x.shape();
    shape.back() = top_k;
    return array::make_arrays(
        {shape, shape},
        {uint32, out_type},
        std::make_shared<MoERoute>(
            s, fallback, top_k, routed_scaling, norm_topk_prob),
        std::move(inputs));
  }
  return fallback(inputs);
}
